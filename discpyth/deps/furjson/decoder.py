...
def _decode_uXXXX(s, pos):  # pylint: disable=invalid-name
    esc = s[pos + 1 : pos + 5]
...
def py_scanstring(  # pylint: disable=dangerous-default-value,invalid-name
...
def JSONObject(  # pylint: disable=invalid-name
    ...
    if is_named_dict_class(obj):
        pairs = obj()
        pairs_append = pairs.__setattr__
    else:
        obj = None
        pairs = []
    ...
    if nextchar != '"':
        ...
        if nextchar == "}":
            if object_pairs_hook is not None:
                ...
                return result, end + 1
            pairs = pairs if obj is not None else {}
    ...
    while True:
        ...
        config = _get_key_config(pairs, key)
        ...
        key = config.attrname if obj is not None else key
        if config.load_hook is not None:
            value = config.load_hook(value)
...
class JSONDecoder(object):
    ...
    def decode(  # pylint: disable=invalid-name
        ...
        """
        obj, end = self.raw_decode(s, obj=obj, idx=_w(s, 0).end())
        end = _w(s, end).end()
        if end != len(s):
            raise JSONDecodeError("Extra data", s, end)
        return obj

    def raw_decode(self, s, obj=None, idx=0):  # pylint: disable=invalid-name
        ...
        """
        try:
            obj, end = self.scan_once(s, idx, obj=obj)
        except StopIteration as err:
            raise JSONDecodeError("Expecting value", s, err.value) from None
        return obj, end
